...
* 총 8개의 **외부 인터럽트**가 존재하며 INT 0 ~ 3핀, INT 4 ~ 7 핀 4개씩 그룹으로 묶여있다. 
	* INT0:3 핀은 PD0 ~ PD3에 매칭된다.
	* INT4:7 핀은 PE4 ~ PE7에 매칭된다.

> EICRA - 외부 인터럽트 제어 레지스터 A

* INT0:3 핀의 동작을 수행할 수 있고 Edge 트리거를 사용할 때 클럭에 상관없이 비동적으로 가능하다.
...
* INT4:7 핀의 동작을 수행할 수 있고 Edge 트리거를 사용할 때 I/O 클럭이 필요하다.
