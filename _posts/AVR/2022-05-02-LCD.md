...
|![blog](https://github.com/JiJinWoo/JiJinWoo.github.io/blob/master/assets/images/avrblog/LCDCircuit.PNG?raw=true)|
|:---:|
...
Instructions Table을 보면 명령어별로 써야하는 데이터와 정보들이 간략하게 설명되어있다.
...
*	Functiuon set을 해준 뒤 39us를 대기해야하지만 편리를 위해 앞으로 1ms을 고정하겠다.
...
void LcdInit(void){	//LCD 초기화 함수
	...
	_delay_ms(2);	//Display clear 후 1.53ms 대기지만 넉넉하게 5ms 대기
```
...
* LCD는 16x2로 가로와 세로이 존재한다. 가로와 세로로 총 32칸 중에 어디에 문자를 쓸지 결정한다.
